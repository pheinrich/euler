...
class Problem_0011
  ...
  N = '08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08 ' \
      ...
      '01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48'.split.map! {|x| x.to_i}
      ...
      for j in 0...h
        ...
        if j < h - n
          if i < w - n
            ...
            p = (0...n).inject(1) {|p, x| p * N[k + x*(w + 1)]}
            max = p if p > max
          end
          ...
          # top to bottom
          p = (0...n).inject(1) {|p, x| p * N[k + x*w]}
          max = p if p > max
        end
        
        if i < w - n
          # left to right
          p = (0...n).inject(1) {|p, x| p * N[k + x]}
          max = p if p > max
        end
      end
    end
